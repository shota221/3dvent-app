...
class SharedUnitsRepository(context: Context) {
    ...
    fun putUnits(units: Units) {
        with(sharedPreferences.edit()) {
            ...
            putString("airFlow", units.airFlow)
            putString("o2Flow", units.o2Flow)
            putString("totalFlow", units.totalFlow)
            putString("estimatedMv", units.estimatedMv)
            putString("airwayPressure", units.airwayPressure)
            putString("estimatedPeep", units.estimatedPeep)
            putString("vtPerKg", units.vtPerKg)
            putString("predictedVt", units.predictedVt)
            putString("estimatedVt", units.estimatedVt)
            putString("i", units.i)
            putString("e", units.e)
            putString("iAvg", units.iAvg)
            putString("eAvg", units.eAvg)
